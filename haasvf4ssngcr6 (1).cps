/**
  ...
  $Revision: 42950 a658a6847fbcfbf4bb3a045932ed25c51ed45033 $
  $Date: 2020-09-28 12:04:01 $
...
function onSection() {
  ...
  executeManualNC();  // execute Manual NC Commands added by jtb 1-5-21
...
function onCyclePoint(x, y, z) {
  ...
  if (forceCycle || isFirstCyclePoint() || isProbeOperation()) {
    ...
    case "probing-xy-circular-hole":
      protectedProbeMove(cycle, x, y, z - cycle.depth);
      // STORE EXPECTED SIZE
      writeBlock("#10800="+xyzFormat.format(cycle.width1));
      writeBlock(
        gFormat.format(65), "P" + 9814,
        "D" + xyzFormat.format(cycle.width1),
        "Q" + xyzFormat.format(cycle.probeOvertravel),
...
description = "HAAS - NextGenControl Inspect Surface";
...
var permittedDprntChars = " ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-";
...
function inspectionCycleInspect(cycle, x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    if (activeG254) {
      ...
      writeBlock("G103 P1 " + formatComment("LOOKAHEAD OFF"));
...
function inspectionProbeTriggerCheck(triggered) {
  ...
  writeBlock("IF [" + inspectionVariables.macroVariable4 + condition + inPositionTolerance + "] GOTO" + skipErrorLine());
...
function skipErrorLine() {
  return (2 * properties.sequenceNumberIncrement + sequenceNumber);
}
...
function inspectionProcessSectionStart() {
  ...
  if (properties.commissioningMode) {
    ...
    writeln("DPRNT[ECCENTRICITY*X****" + macroFormat.format(properties.probeEccentricityX) + outputFormat + "]");
    writeln("DPRNT[ECCENTRICITY*Y****" + macroFormat.format(properties.probeEccentricityY) + outputFormat + "]");
    forceSequenceNumbers(true);
    writeBlock("IF [" + inspectionVariables.probeRadius + " NE #0] GOTO" + skipErrorLine());
    writeBlock("#3000 = 1" + formatComment("PROBE NOT CALIBRATED OR PROPERTY CALIBRATED RADIUS INCORRECT"));
    writeBlock("IF [" + inspectionVariables.probeRadius + " NE 0] GOTO" + skipErrorLine());
    writeBlock("#3000 = 1" + formatComment("PROBE NOT CALIBRATED OR PROPERTY CALIBRATED RADIUS INCORRECT"));
    writeBlock("IF [" + inspectionVariables.probeRadius + " LT " + xyzFormat.format(tool.diameter / 2) + "] GOTO" + skipErrorLine());
    writeBlock("#3000 = 1" + formatComment("PROBE NOT CALIBRATED OR PROPERTY CALIBRATED RADIUS INCORRECT"));
    var maxEccentricity = (unit == MM) ? 0.2 : 0.0079;
    writeBlock("IF [ABS[" + macroFormat.format(properties.probeEccentricityX) + "] LT " + maxEccentricity + "] GOTO" + skipErrorLine());
    writeBlock("#3000 = 1" + formatComment("PROBE NOT CALIBRATED OR PROPERTY ECCENTRICITY X INCORRECT"));
    writeBlock("IF [ABS[" + macroFormat.format(properties.probeEccentricityY) + "] LT " + maxEccentricity + "] GOTO" + skipErrorLine());
    writeBlock("#3000 = 1" + formatComment("PROBE NOT CALIBRATED OR PROPERTY ECCENTRICITY Y INCORRECT"));
    writeBlock("IF [" + macroFormat.format(properties.probeEccentricityX) + " NE #0] GOTO" + skipErrorLine());
    writeBlock("#3000 = 1" + formatComment("PROBE NOT CALIBRATED OR PROPERTY ECCENTRICITY X INCORRECT"));
    writeBlock("IF [" + macroFormat.format(properties.probeEccentricityY) + " NE #0] GOTO" + skipErrorLine());
    writeBlock("#3000 = 1" + formatComment("PROBE NOT CALIBRATED OR PROPERTY ECCENTRICITY Y INCORRECT"));
    writeBlock(" ");
    forceSequenceNumbers(false);
  }
...
function inspectionCreateResultsFileHeader() {
  ...
  resFile = filterText(resFile, permittedDprntChars);
  // replace spaces with minus sign as controllers don't like spaces in filenames
  ...
  if (isInspectionOperation(currentSection)) {
    ...
    if ((!properties.singleResultsFile) || (inspectionVariables.inspectionSectionCount == inspectionVariables.inspectionSections)) {
      ...
      if (properties.commissioningMode) {
        if (controlType == "NGC") {
          forceSequenceNumbers(true);
          writeBlock(inspectionVariables.macroVariable1 + " = [#20261 * " + 4 * properties.sequenceNumberIncrement + " + " + skipErrorLine() + "]");
          writeBlock("GOTO " + inspectionVariables.macroVariable1);
          writeBlock(" ");
          writeBlock("#3006=1" + formatComment("DPRNT LOCATION NOT SET"));
          onCommand(COMMAND_STOP);
          writeBlock("GOTO " + (8 * properties.sequenceNumberIncrement + sequenceNumber));
          writeBlock(" ");
          writeBlock("#3006=1" + formatComment("CHECK SETTING 262 FOR RESULTS FILE LOCATION"));
          onCommand(COMMAND_STOP);
          writeBlock("GOTO " + (4 * properties.sequenceNumberIncrement + sequenceNumber));
          writeBlock(" ");
    ...
    writeBlock("G103 P0 " + formatComment("LOOKAHEAD ON"));
  }
}
